/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  IsInt,
  Min,
  Max,
  ValidateNested,
} from "class-validator";
import { RequestLogCreateNestedManyWithoutUrlsInput } from "./RequestLogCreateNestedManyWithoutUrlsInput";
import { Type } from "class-transformer";
import { RequestParameterCreateNestedManyWithoutUrlsInput } from "./RequestParameterCreateNestedManyWithoutUrlsInput";

@InputType()
class UrlCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  address?: string | null;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  interval?: number | null;

  @ApiProperty({
    required: false,
    type: () => RequestLogCreateNestedManyWithoutUrlsInput,
  })
  @ValidateNested()
  @Type(() => RequestLogCreateNestedManyWithoutUrlsInput)
  @IsOptional()
  @Field(() => RequestLogCreateNestedManyWithoutUrlsInput, {
    nullable: true,
  })
  requestLogs?: RequestLogCreateNestedManyWithoutUrlsInput;

  @ApiProperty({
    required: false,
    type: () => RequestParameterCreateNestedManyWithoutUrlsInput,
  })
  @ValidateNested()
  @Type(() => RequestParameterCreateNestedManyWithoutUrlsInput)
  @IsOptional()
  @Field(() => RequestParameterCreateNestedManyWithoutUrlsInput, {
    nullable: true,
  })
  requestParameters?: RequestParameterCreateNestedManyWithoutUrlsInput;
}

export { UrlCreateInput as UrlCreateInput };
